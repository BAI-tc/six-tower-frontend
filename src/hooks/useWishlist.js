...
export function WishlistButton({ game }) {
  ...
  return (
    <button
      onClick={handleClick}
      disabled={loading}
      className="p-2.5 bg-black/60 hover:bg-red-500 rounded-full transition-all duration-300 shadow-lg backdrop-blur-sm border border-white/10"
      title={status.in_wishlist ? '从愿望单移除' : '添加到愿望单'}
    >
      <svg
        className="w-5 h-5 text-white"
        fill={status.in_wishlist ? 'currentColor' : 'none'}
        stroke="currentColor"
        viewBox="0 0 24 24"
      >
