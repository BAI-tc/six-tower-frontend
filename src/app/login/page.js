...
export default function LoginPage() {
  ...
  const handleDirectLogin = async () => {
    ...
    if (isWishlistUrl(steamId)) {
      setError('请输入有效的 Steam ID 或 Steam 个人主页链接（愿望单链接暂不支持）');
      setIsLoading(false);
      return;
    }
      ...
      <div className='bg-black/80 backdrop-blur-sm p-8 rounded-2xl border border-slate-700 max-w-md w-full mx-4'>
        ...
        <div className='mb-4'>
          ...
          <input
            ...
            placeholder='Steam ID (如 76561198028184818) 或链接'
        ...
        <p className='text-xs text-slate-500 text-center mt-6'>
          支持 Steam ID (如 76561198028184818) 或 Steam 个人主页 URL
        </p>
      </div>
    </div>
    </>
